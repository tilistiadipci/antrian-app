    ...
    <div class="container">
        <div class="row">
            <div class="col s12 m6">
                ...
                <div class="card">
                    <span class="btn blue right" style="margin:10px 20px 0px;"><i class="mdi-social-people left"style="margin:-1px 5px 0px;"></i><span class="badge" style="color:#fff;"></span></span>
                ...
                <span style="font-size:30px">{{ trans('messages.call.number') }} {{ trans('messages.display.token') }}</span><br>
                <span id="num0" style="font-size:125px;color:red;font-weight:bold;line-height:1.0">@if($data[0]['user_id']==$user->id){{ $data[0]['layanan'] }}{{ $data[0]['number'] }}@endif </span><br>
                        ...
                        <form id="new_call" action="{{ route('post_call') }}" method="post">
                                    ...
                                    <select id="counter" class="browser-default" name="counter" data-error=".counter">
                                        ...
                                        @foreach($counters as $counter)
                                            @if(session()->has('counter') && ($counter->id==session()->get('counter')))
                                                <option value="{{ $counter->id }}" selected>{{ $counter->name }} {{ $counter->idcounter }}</option>
                                            @else
                            ...
                            <div class="row">
                                ...
                                <div class="input-field col s12" style="display:none;">
                                    ...
                                    <select id="department" class="browser-default" name="department" data-error=".department">
                                        @foreach($departments as $department)
                                            @if(session()->has('department') && ($department->id==session()->get('department')))
                                                <option value="{{ $user->departments }}" selected>{{ $department->name }}</option>
                                            @else
                                                <option value="{{ $user->departments }}">{{ $department->name }}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                    <div class="department">
                            ...
                            <div class="row">
                                <div class="input-field col s12" style="display:none;">
                                    ...
                                    <select id="counter" class="browser-default" name="counter" data-error=".counter">
                                        @foreach($counters as $counter)
                                            @if(session()->has('counter') && ($counter->id==session()->get('counter')))
                                                <option value="{{ $user->counter }}" selected>{{ $counter->name }} {{ $counter->idcounter }}</option>
                                            @else
                                                <option value="{{ $user->counter }}">{{ $user->counter }} {{ $counter->idcounter }}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                    <div class="counter">
                            ...
                            <div class="row">
                                @if(!$user->is_admin)
                                <div class="input-field col s12">
                                    <button class="btn btn-large waves-effect waves-light center" type="submit" style="width:100%;background:#555;border-radius: 20px 20px 20px 20px;">
            ...
            <div class="col s12 m6">
                ...
                <td>
                                ...
                                Testimoni<i class="mdi-maps-rate-review left"></i>
                                        ...
                                        <a class="btn waves-effect red right frmsubmit" href="{{ route('logout') }}" message="false" method="post">
